...
void* StackAllocator::allocate(size_t size, size_t alignment) {
    ...
    // ペイロード直前に prevOffset を書き込む（memcpy でアライメント非依存に書き込む）
    std::memcpy(reinterpret_cast<void*>(payloadAddr - sizeof(size_t)), &m_offset, sizeof(size_t));
...
void StackAllocator::deallocate(void* ptr) {
    ...
    // ペイロードの直前 sizeof(size_t) バイトに格納された prevOffset を読み出す（memcpy でアライメント非依存に読む）
    size_t prevOffset;
